...
class MapsController extends GetxController {
  ...
  void startLiveLocation() async {
    try {
      ...
      mapController.value.move(latLng, zoomLevel);

      // Perhitungan jarak dan polygon
    ...
    // Setelah itu baru mulai live tracking
  ...
  Future<void> getCurrentLocation() async {
    try {
      ...
      final latLng = LatLng(position.latitude, position.longitude);
      // final latLng = LatLng(-6.1960200240581475, 106.9787514224421);
      currentPosition.value = latLng;

      // Pindahkan peta ke lokasi baru!
      mapController.value.move(latLng, 16.0); // tambahkan ini
      ...
      final isInside = isPointInPolygon(latLng, polygonPoints);
      isNearLocation.value = isInside;
      // isNearLocation.value = distance <= 500;
      isNearLocationBefore.value = distance <= 1000;
    } catch (e) {
    ...
    if (response.isSuccess) {
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                  content:
                      Text(addressModel.value?.displayName ?? '')),
            );
          });
